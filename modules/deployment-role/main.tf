...
resource "aws_iam_role" "deployment" {
  name               = "gharole-${var.project_name}-${var.environment == "Production" ? "prd" : lower(substr(var.environment, 0, 3))}"
...
resource "aws_iam_policy" "deployment" {
  name        = "ghpolicy-${var.project_name}-${var.environment == "Production" ? "prd" : lower(substr(var.environment, 0, 3))}"
...
resource "aws_ssm_parameter" "role_arn" {
  name        = "/deployment-roles/${replace(var.github_repository, "/", "-")}/role-arn"
  type        = "String"
  value       = aws_iam_role.deployment.arn
  description = "Deployment role ARN for ${var.github_repository}"
  tags        = var.tags
}
